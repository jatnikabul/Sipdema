...
class PelayananSuratsController extends Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->menu         = trans('menu.user.name');
        $this->route        = $this->routes['backend'].'pelayanan_surat';
        $this->slug         = $this->slugs['backend'].'pelayanan_surat';
        $this->view         = $this->views['backend'].'pelayanan_surat';
        $this->breadcrumb   = '<li><a href="'.route($this->route.'.index').'">'.$this->menu.'</a></li>';
        #share parameters
        $this->share();
        # init model
        $this->model = new \App\Models\PelayananSurat;
